			...
			<tbody>
				<xsl:for-each select="descendant::DataGroup[not(@hidden='true')]">
					...
					<xsl:if test="Use = $messageType">
						<tr>
							...
							<td>
								<xsl:choose>
									...
									<xsl:when test="MinOccurrence[@use=$messageType]=0">O</xsl:when>
									<xsl:when test="MinOccurrence[@use=$messageType]>0">R</xsl:when>
									<xsl:when test="MinOccurrence[not(@*)]>0">R</xsl:when>
									<xsl:otherwise>O</xsl:otherwise>
								</xsl:choose>
							</td>
							<td>
								<xsl:choose>
									<xsl:when test="MaxOccurrence[@use=$messageType] != ''">
										<xsl:value-of select="MaxOccurrence[@use=$messageType]"/>x</xsl:when>
									<xsl:otherwise>
										<xsl:value-of select="MaxOccurrence"/>x</xsl:otherwise>
								</xsl:choose>
							</td>
							<td>
						...
						<xsl:if test="Description[@lang=$language and @use=$messageType] | Description[@lang=$language and not(@use)]!=''">
							<tr>
								<xsl:attribute name="class">
								...
								<td colspan="100%">
									...
									<xsl:for-each select="Description[@lang=$language and not(@use)] | Description[@lang=$language and @use=$messageType]">
										<xsl:choose>
				...
				<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:if test="Use = $messageType">
							...
							<xsl:if test="Description[@lang=$language and @use=$messageType] | Description[@lang=$language and not(@use)]!='' or Hyperlink[@lang=$language]!=''">
								<tr>
									<xsl:choose>
										<xsl:when test="count(ancestor::*)>2">
											<xsl:attribute name="class">
